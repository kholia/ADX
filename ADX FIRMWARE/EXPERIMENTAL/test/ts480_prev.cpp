...
void setFreqCAT() {
  ...
  #ifdef DEBUG
     _INFOLIST("%s CAT Received frequency f=%ld\n",__func__,fx);
  #endif //DEBUG

  
  int      b = setSlot(fx);
  #ifdef DEBUG
     _INFOLIST("%s CAT setSlot b=%d\n",__func__,b);
  #endif //DEBUG
  ...
  if (b != Band_slot) { //band change
    ...
    freq = fx;
    #ifdef DEBUG
       _INFOLIST("%s b=%d Band_slot=%d different new Freq_assigned\n",__func__,b,Band_slot);
    #endif //DEBUG
  }
  ...
  */
  ...
  int k = Bands[j];
  #ifdef DEBUG
     _INFOLIST("%s CAT Bands[%d]=%d\n",__func__,j,k);
  #endif //DEBUG

  int q = band2Slot(k);
  #ifdef DEBUG
     _INFOLIST("%s CAT band2slot(%d)=%d\n",__func__,k,q);
  #endif //DEBUG

  int m = getMode(q, freq);
  #ifdef DEBUG
     _INFOLIST("%s CAT getMode(%d,%ld)=%d\n",__func__,q,freq,m);
  #endif //DEBUG
  ...
  if (getWord(SSW, CWMODE) == false) {
    ...
    if (mode != m) {
      ...
      Mode_assign();
      #ifdef DEBUG
        _INFOLIST("%s CAT mode changed mode=%d m=%d freq=%ld\n",__func__,mode,m,freq);
      #endif //DEBUG
...
void serialEvent() {
...
#ifdef DEBUG
  _TRACELIST("%s Serial reading len(%d)=%s\n", __func__,rc,buf);
#endif //DEBUG  
...
#ifdef DEBUG
  _TRACELIST("%s CAT completed buffer=%s len=(%d)\n", __func__, serialBuffer,k);
#endif //DEBUG  
  ...
  if (strcmp((const char*)serialBuffer, strCmd) == 0) { //coincidence
    ...
    for (int i = 0; i < k; i++) {
      ...
      CATcmd[cat_ptr++] = data;
#ifdef DEBUG
      _TRACELIST("%s Transferred to CAT buffer data=%c CATcmd[%d]=%c\n", __func__, data, i, CATcmd[i]);
#endif //DEBUG
...
#ifdef DEBUG
        _TRACELIST("%s() CAT cmd(%s) ready to process\n", __func__, CATcmd);
#endif //DEBUG
