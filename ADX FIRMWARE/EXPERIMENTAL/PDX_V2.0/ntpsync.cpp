...
bool setClock(char* n1,char* n2) {
  ...
  #ifdef DEBUG
     _INFOLIST("%s NTP time is %s",__func__,asctime(&timeinfo));
  #endif //DEBUG
  return ntp_rc;
}
/*------------
...
int checkAP(char* s,char* p) {
  ...
  #ifdef DEBUG
     _INFOLIST("%s Verifying connection with ssid(%s)\n",__func__,s);
  #endif //DEBUG
  ...
  #ifdef DEBUG
     _INFOLIST("%s Connecting id(%s) password(%s)\n",__func__,s,p);
  #endif //DEBUG
...
*/
  ...
  if (checkAP((char*)&wifi.ssid,(char*)&wifi.password) != WL_CONNECTED) {
     return SYNC_NO_AP;     
  }
